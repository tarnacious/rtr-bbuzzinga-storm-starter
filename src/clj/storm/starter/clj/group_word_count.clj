...
(defspout zmq-line-spout ["words"]
  ...
  (let [socket (-> 1 mq/context (mq/socket mq/req))]
    (mq/connect socket "tcp://localhost:5556")
    (spout
     (nextTuple []
       (mq/send socket "Hello\u0000")
       (let [string (mq/recv-str socket)]
...
(defn mk-topology []
  (topology
   {"1" (spout-spec test-line-spout)}
   {"2" (bolt-spec {"1" ["words"]} word-bolt :p 5)
